...
Requires: pip install virt-graph[handlers]
...
try:
    ...
    def _missing_deps(*args, **kwargs):
        raise ImportError(
            "Handler dependencies not installed. "
            "Install with: pip install virt-graph[handlers]"
        )

    traverse = traverse_collecting = path_aggregate = _missing_deps
    shortest_path = all_shortest_paths = _missing_deps
    centrality = connected_components = graph_density = _missing_deps
    neighbors = resilience_analysis = _missing_deps
