...
class FMCGDataGenerator:
    ...
    def __init__(self, seed: int = RANDOM_SEED):
        ...
        self.supplier_ingredient_ids: dict[
            tuple, int
        ] = {}  # (supplier_id, ingredient_id) -> id
        ...
        self.inventory_ids: dict[
            tuple, int
        ] = {}  # (location_type, location_id, sku_id) -> id

        # Level 8-9: Demand and orders
        self.order_ids: dict[str, int] = {}  # order_number -> id
        self.demand_forecast_ids: dict[
            tuple, int
        ] = {}  # (sku_id, location_id, period) -> id
        ...
        # =================================================================
        # Phase 6: Chaos Injection Module Integration
        # =================================================================

        # RiskEventManager: Risk event triggering and override configuration
        self.risk_manager = RiskEventManager(
            manifest_path=BENCHMARK_MANIFEST_PATH,
            seed=seed,
        )

        # QuirksManager: Behavioral pathology injection
        self.quirks_manager = QuirksManager(
            manifest_path=BENCHMARK_MANIFEST_PATH,
            seed=seed,
        )

        # Trigger all risk events deterministically for testing
        self.triggered_events = self.risk_manager.trigger_all()

        # Set stochastic mode based on port strike event
        self.stochastic_mode = self.risk_manager.get_stochastic_mode()
    ...
    def _generate_level_0(self) -> None:
        ...
        emission_overrides = self.risk_manager.get_emission_overrides()
        ...
        print(
            f"    Generated: {len(self.data['divisions'])} divisions, {len(self.data['channels'])} channels, "
    ...
    def _generate_level_1(self) -> None:
        ...
        dc_data = {
            dc["code"]: (self.dc_ids.get(dc["code"]), dc)
            for dc in DCS
            if dc["code"] in self.dc_ids or True
        }
        ...
        print(
            f"    Generated: {len(self.data['suppliers'])} suppliers, {len(self.data['plants'])} plants, "
    ...
    def _generate_level_2(self) -> None:
        ...
        supplier_overrides = self.risk_manager.get_supplier_overrides()
        palm_otd = supplier_overrides.get(
            "degraded_otd_rate", 0.50
        )  # Default already poor
        ...
        self.data["supplier_ingredients"].append(
            {
                ...
                "on_time_delivery_rate": palm_otd,  # Degraded by RSK-SUP-003 if triggered
        ...
        for ing_id in ingredient_ids:
            ...
            num_suppliers = random.choices([1, 2, 3, 4, 5], weights=[5, 35, 40, 15, 5])[
                0
            ]
                    ...
                    self.data["formulas"].append(
                        {
                            ...
                            "mix_time_minutes": prod_info["mix_time"]
                            + random.randint(-5, 5),
    ...
    def _generate_level_3(self) -> None:
        ...
        for i in range(4500):
            ...
            nam_dcs = [
                dc_id
                for dc_code, dc_id in self.dc_ids.items()
            ...
            self.data["retail_locations"].append(
                {
                    ...
                    "store_format": random.choices(
                        store_formats[:3], weights=[40, 50, 10]
                    )[0],
                    "country": "USA",
                    "city": city,
        ...
        for _ in range(remaining):
            ...
            primary_dc = (
                random.choice(div_dcs) if div_dcs else random.choice(dc_ids_list)
            )
    ...
    def _generate_level_4(self) -> None:
        ...
        for prod_code, pkg_codes in product_packaging.items():
            ...
            prod_formulas = [
                fid
                for fc, fid in self.formula_ids.items()
            ...
            self.data["skus"].append(
                {
                    ...
                    "product_id": prod_id,
                    "packaging_id": pkg_id,
                    "formula_id": random.choice(prod_formulas)
                    if prod_formulas
                    else None,
                    "region": region,
                ...
                self.data["promotion_accounts"].append(
                    {
                        "promo_id": promo["id"],
                        "retail_account_id": acct_id,
                        "trade_spend_allocation": round(
                            promo["trade_spend_budget"] / len(promo_accounts), 2
                        ),
                        "created_at": now,
                    }
                )
        ...
        print(
            f"    Generated: {len(self.data['skus'])} SKUs, {len(self.data['promotions'])} promotions, "
    ...
    def _generate_level_5(self) -> None:
        ...
        for po_id in range(1, 25001):
            ...
            if days_ago > 60:
                ...
                status = random.choices(po_statuses, weights=[0, 5, 10, 20, 20, 40, 5])[
                    0
                ]
            else:
                status = random.choices(po_statuses, weights=[5, 15, 30, 25, 15, 5, 5])[
                    0
                ]
        ...
        received_pos = [
            ...
            if po["status"] in ["received", "partial", "shipped"]
        ]
        gr_id = 1
        ...
        for wo_id in range(1, 50001):
            ...
            plant_lines = [
                lid
                for lc, lid in self.production_line_ids.items()
                if any(
                    l["plant_id"] == plant_id and l["id"] == lid
                    for l in self.data["production_lines"]
                )
            ]
            line_id = (
                random.choice(plant_lines) if plant_lines else random.choice(line_ids)
            )

            planned_start = self.fake.date_between(
                start_date=date(self.base_year, 1, 1),
                end_date=date(self.base_year, 12, 20),
            )
            ...
            actual_qty = (
                round(planned_qty * random.uniform(0.95, 1.02), 2)
                if status == "completed"
                else None
            )
            ...
            self.data["work_orders"].append(
                {
                    ...
                    "actual_start_datetime": datetime.combine(
                        planned_start, datetime.min.time()
                    )
                    if status in ["in_progress", "completed"]
                    else None,
                    "actual_end_datetime": datetime.combine(
                        planned_end, datetime.min.time()
                    )
                    if status == "completed"
                    else None,
                    "status": status,
            ...
            self.data["supplier_esg_scores"].append(
                {
                    "id": sup["id"],
                    "supplier_id": sup["id"],
                    "assessment_date": self.fake.date_between(
        ...
        for ms_id in range(1, 101):
            ...
            target_mode = (
                "rail" if current_mode == "truck" else random.choice(["truck", "ocean"])
            )
        ...
        print(
            f"    Generated: {len(self.data['purchase_orders'])} POs, {len(self.data['goods_receipts'])} GRs, "
    ...
    def _generate_level_6(self) -> None:
        ...
        po_lines_idx = LookupBuilder.build_po_lines_by_po_id(
            self.data["purchase_order_lines"]
        )
        ...
        formula_ings_idx = LookupBuilder.build_formula_ings_by_formula_id(
            self.data["formula_ingredients"]
        )
        ...
        for wo in self.data["work_orders"]:
            ...
            for fi in formula_ings[:5]:  # Limit to 5 key materials per WO
                planned_qty = fi["quantity_kg"] * (
                    wo["planned_quantity_kg"] / formula["batch_size_kg"]
                )
                actual_qty = (
                    round(planned_qty * random.uniform(0.98, 1.03), 4)
        ...
        self.data["batches"].append(
            {
                ...
                "quantity_kg": recall_wo["actual_quantity_kg"]
                or recall_wo["planned_quantity_kg"],
                "yield_percent": 97.5,
            ...
            self.data["batches"].append(
                {
                    ...
                    "quantity_kg": wo["actual_quantity_kg"]
                    or round(wo["planned_quantity_kg"] * random.uniform(0.95, 1.02), 2),
                    "yield_percent": round(random.uniform(95, 99.5), 2),
                    "production_date": prod_date,
                    "expiry_date": expiry,
                    "qc_status": random.choices(qc_statuses, weights=[5, 88, 2, 3, 2])[
                        0
                    ],
                    "qc_release_date": prod_date + timedelta(days=random.randint(1, 5))
        ...
        # =================================================================
        # Phase 6: Chaos Injection - Batch-level modifications
        # =================================================================

        # RSK-BIO-001: Contamination - mark affected batches as REJECTED
        batch_overrides = self.risk_manager.get_batch_overrides()
        if batch_overrides:
            target_ing = batch_overrides.get("target_ingredient", "ING-SORB-001")
            contaminated_count = 0
            # The recall batch B-2024-RECALL-001 already marked with hold status
            # Upgrade to REJECTED for the risk event
            for batch in self.data["batches"]:
                if batch.get("is_contaminated"):
                    batch["qc_status"] = batch_overrides.get(
                        "qc_status_override", "REJECTED"
                    )
                    batch["qc_notes"] = (
                        f"RISK EVENT RSK-BIO-001: {target_ing} contamination - {batch['qc_notes']}"
                    )
                    contaminated_count += 1
            if contaminated_count > 0:
                print(
                    f"    [Risk] RSK-BIO-001: {contaminated_count} contaminated batches marked REJECTED"
                )

        # Data decay quirk: older batches have higher rejection rates
        if self.quirks_manager.is_enabled("data_decay"):
            reference_date = datetime.now()
            self.data["batches"] = self.quirks_manager.apply_data_decay(
                self.data["batches"],
                reference_date=reference_date,
            )
            # Count affected batches for reporting
            decay_count = sum(
                1 for b in self.data["batches"] if b.get("data_decay_affected")
            )
            if decay_count > 0:
                print(
                    f"    [Quirk] Data decay applied: {decay_count} batches marked for rejection"
                )

        # Single source fragility quirk: apply lead time cascade for SPOF ingredients
        if self.quirks_manager.is_enabled("single_source_fragility"):
            params = self.quirks_manager.get_params("single_source_fragility")
            delay_multiplier = params.get("cascade_delay_multiplier", 2.5)

            # Find formulas that use SPOF ingredients (Palm Oil or Sorbitol)
            spof_ing_ids = {
                self.ingredient_ids.get("ING-PALM-001"),
                self.ingredient_ids.get("ING-SORB-001"),
            }
            spof_formula_ids = set()
            for fi in self.data.get("formula_ingredients", []):
                if fi.get("ingredient_id") in spof_ing_ids:
                    spof_formula_ids.add(fi.get("formula_id"))

            # Apply delay to batches using SPOF formulas
            fragility_count = 0
            for batch in self.data["batches"]:
                if batch.get("formula_id") in spof_formula_ids:
                    # Mark as SPOF-affected (the actual delay is informational)
                    batch["spof_affected"] = True
                    fragility_count += 1

            if fragility_count > 0:
                print(f"    [Quirk] Single source fragility: {fragility_count} batches use SPOF ingredients")
    ...
    def _generate_level_7(self) -> None:
        ...
        for batch in self.data["batches"]:
            ...
            for fi in formula_ings:
                ...
                scrap = round(
                    max(0, actual_qty - planned_qty) * random.uniform(0, 0.5), 4
                )
        ...
        for dc_id in dc_ids:
            ...
            for sku_id in dc_skus:
                ...
                available_batches = random.sample(
                    batch_ids, min(num_lots, len(batch_ids))
                )
                    ...
                    self.data["inventory"].append(
                        {
                            ...
                            "receipt_date": batch["production_date"]
                            + timedelta(days=random.randint(1, 14)),
                            "aging_bucket": random.choice(
                                ["0-30", "31-60", "61-90", "90+"]
                            ),
        ...
        # =================================================================
        # Phase 6: Chaos Injection - Inventory-level modifications
        # =================================================================

        # Phantom inventory quirk: 2% shrinkage with detection lag
        if self.quirks_manager.is_enabled("phantom_inventory"):
            reference_date = datetime.now()
            self.data["inventory"] = self.quirks_manager.apply_phantom_inventory(
                self.data["inventory"],
                reference_date=reference_date,
            )
            # Count affected inventory records
            shrinkage_count = sum(
    ...
    def _generate_level_8(self) -> None:
        ...
        # --- ORDERS (~200,000) ---
        # Phase 5: Build O(1) lookup indices (replaces O(N) scans)
        locations_by_account_idx = LookupBuilder.build_locations_by_account_id(
            self.data["retail_locations"]
        )
        ...
        for acct_id in account_ids:
            ...
            location_ids_by_account[acct_id] = (
                [loc["id"] for loc in locs] if locs else location_ids
            )
            ...
            self.data["orders"].append(
                {
                    ...
                    + timedelta(days=lead_time + random.randint(-1, 3)),
                    "actual_delivery_date": order_date
                    + timedelta(days=lead_time + random.randint(-2, 5))
                    if random.random() > 0.2
                    else None,
                    "status": random.choices(
                        order_statuses, weights=[5, 10, 10, 10, 15, 45, 5]
                    )[0],
            ...
            self.data["demand_allocation"].append(
                {
                    ...
                    "allocation_rule": random.choice(
                        ["fair_share", "priority", "historical", "committed"]
                    ),
                    "status": random.choice(
                        ["pending", "confirmed", "released", "completed"]
                    ),
                    "created_at": now,
                    "updated_at": now,
                }
            ...
            self.data["capacity_plans"].append(
                {
                    ...
                    "production_line_id": random.choice(
                        list(self.production_line_ids.values())
                    ),
        ...
        # =================================================================
        # Phase 6: Chaos Injection - Demand-level modifications
        # =================================================================

        # Human optimism bias quirk: over-forecast new products by 15%
        if self.quirks_manager.is_enabled("human_optimism_bias"):
            # Build SKU launch dates (approximate: assume SKUs < 6 months old are "new")
            # For simplicity, mark ~10% of SKUs as new products
            new_sku_ids = set(list(self.sku_ids.values())[: len(self.sku_ids) // 10])
            sku_launch_dates = {
                sku_id: datetime.now() - timedelta(days=random.randint(30, 180))
                for sku_id in new_sku_ids
            }

            self.data["demand_forecasts"] = self.quirks_manager.apply_optimism_bias(
                self.data["demand_forecasts"],
                sku_launch_dates=sku_launch_dates,
                reference_date=datetime.now(),
            )
            # Count affected forecasts
            bias_count = sum(
                1
                for f in self.data["demand_forecasts"]
                if f.get("optimism_bias_applied")
            )
            if bias_count > 0:
                print(
                    f"    [Quirk] Optimism bias applied: {bias_count} forecasts inflated by 15%"
                )

        self.generated_levels.add(8)
        level_elapsed = time.time() - level_start
        print(
            f"    Generated: {len(self.data['pos_sales'])} pos_sales, {len(self.data['orders'])} orders, "
    ...
    def _generate_level_9(self) -> None:
        ...
        order_channel_ids = np.array(
            [o["channel_id"] for o in self.data["orders"]], dtype=np.int64
        )
        ...
        if default_mask.any():
            lines_per_order[default_mask] = self.rng.integers(
                5, 21, size=default_mask.sum()
            )
        ...
        released_batches = [
            b for b in self.data["batches"] if b["qc_status"] == "released"
        ]
        batch_ids_list = (
            [b["id"] for b in released_batches] if released_batches else [1]
        )
                ...
                if random.random() < 0.70:
                    ...
                    for split_idx in range(num_splits):
                        if split_idx == num_splits - 1:
                            ...
                            split_qty = random.randint(
                                1, max(1, remaining - (num_splits - split_idx - 1))
                            )
                            remaining -= split_qty
        ...
        for dc_id in dc_ids:
            ...
            for wave_idx in range(num_waves_for_dc):
                ...
                wave_status = random.choices(
                    wave_statuses, weights=wave_status_weights
                )[0]
            ...
            for sku_id in sampled_skus:
                for dest_dc_id in sampled_dcs:
                    source_type = random.choices(
                        source_types, weights=source_type_weights
                    )[0]
                    ...
                    if source_type == "production":
                        ...
                        source_id = (
                            random.choice(other_dcs) if other_dcs else dest_dc_id
                        )
                    ...
                    plan_status = random.choices(
                        plan_statuses, weights=plan_status_weights
                    )[0]
        ...
        root_causes = {
            "demand_spike": [
                "Promotional lift exceeded forecast",
                "Competitor stockout",
                "Viral social media",
            ],
            "capacity_shortage": [
        ...
        recommended_actions = {
            "demand_spike": [
                "Expedite production",
                "Reallocate from other DCs",
                "Authorize overtime",
            ],
            "capacity_shortage": [
                "Shift to alternate plant",
                "Use co-packer",
                "Delay non-critical orders",
            ],
            "material_shortage": [
                "Qualify alternate supplier",
                "Use substitute ingredient",
                "Air freight",
            ],
            "lead_time_violation": [
            ...
            "inventory_excess": [
                "Run promotion",
                "Transfer to high-demand DC",
                "Donate before expiry",
            ],
            "supply_disruption": [
                "Activate business continuity plan",
                "Source from alternate region",
                "Ration allocation",
            ],
        }
        ...
        for week_num in range(1, 53):
            ...
            for _ in range(num_exceptions):
                exc_type = random.choices(
                    exception_types, weights=exception_type_weights
                )[0]
                severity = random.choices(severities, weights=severity_weights)[0]
                exc_status = random.choices(
                    exception_statuses, weights=exception_status_weights
                )[0]
                ...
                if exc_type == "material_shortage" and random.random() < 0.1:
                    root_cause = (
                        "Single-source Sorbitol (ING-SORB-001) supplier constraint"
                    )
                    severity = "high" if random.random() < 0.5 else "critical"
                elif exc_type == "lead_time_violation" and random.random() < 0.1:
                    root_cause = (
                        "Palm Oil (ING-PALM-001) extended lead time from Malaysia"
                    )
        ...
        # =================================================================
        # Phase 6: Chaos Injection - Order/Picking-level modifications
        # =================================================================

        # RSK-CYB-004: Cyber outage - set Chicago DC pick waves to ON_HOLD
        dc_overrides = self.risk_manager.get_dc_overrides()
        if dc_overrides:
            target_dc_code = dc_overrides.get("target_dc", "DC-NAM-CHI-001")
            target_dc_id = self.dc_ids.get(target_dc_code)
            if target_dc_id:
                hold_count = 0
                for wave in self.data["pick_waves"]:
                    if wave.get("dc_id") == target_dc_id:
                        wave["status"] = "on_hold"
                        wave["notes"] = (
                            f"RISK EVENT RSK-CYB-004: WMS outage - {dc_overrides.get('hold_duration_hours', 72)}h hold"
                        )
                        hold_count += 1
                if hold_count > 0:
                    print(
                        f"    [Risk] RSK-CYB-004: {hold_count} pick waves at {target_dc_code} set to ON_HOLD"
                    )

        # Bullwhip quirk: batch orders during promo weeks (applied at order_lines level)
        # Note: This modifies order line quantities to create larger variance
        if self.quirks_manager.is_enabled("bullwhip_whip_crack"):
            # Identify promo orders (using Black Friday promo)
            bf_promo_id = self.promotion_ids.get("PROMO-BF-2024")
            promo_order_ids = set()
            for order in self.data["orders"]:
            ...
            if promo_order_ids:
                self.data["orders"], self.data["order_lines"] = (
                    self.quirks_manager.apply_bullwhip(
                        self.data["orders"],
                ...
                print(
                    f"    [Quirk] Bullwhip applied: {len(promo_order_ids)} promo orders batched (3x quantities)"
                )
    ...
    def _generate_level_10(self) -> None:
        ...
        route_ids = (
            [r["id"] for r in self.data["routes"]] if self.data["routes"] else [1]
        )
        segment_ids = (
            list(self.route_segment_ids.values()) if self.route_segment_ids else [1]
        )
        ...
        for _ in range(180000):
            ...
            shipment_type = random.choices(
                shipment_types, weights=shipment_type_weights
            )[0]
            ...
            status = random.choices(shipment_statuses, weights=shipment_status_weights)[
                0
            ]
        ...
        # =================================================================
        # Phase 6: Chaos Injection - Logistics-level modifications
        # =================================================================

        # RSK-LOG-002: Port strike - Apply additional delays to affected port legs
        port_overrides = self.risk_manager.get_port_strike_overrides()
        if port_overrides:
            affected_port_codes = port_overrides.get("affected_ports", ["USLAX"])
            delay_multiplier = port_overrides.get("delay_multiplier", 4.0)

            # Build port_id -> port_code lookup
            port_id_to_code = {v: k for k, v in self.port_ids.items()}
            affected_port_ids = {
                self.port_ids.get(code)
                for code in affected_port_codes
                if code in self.port_ids
            }

            # Build segment_id -> segment lookup for O(1) access
            ...
            for leg in self.data["shipment_legs"]:
                ...
                if seg_id and seg_id in segment_lookup:
                    ...
                    # Check if segment origin or destination is an affected port
                    origin_is_port = (
                    ...
                    if origin_is_port or dest_is_port:
                        ...
                        leg["actual_transit_hours"] = round(
                            current_transit + extra_delay, 2
                        )
                        leg["status"] = "delayed"
                        leg["notes"] = (
                            f"RISK EVENT RSK-LOG-002: Port strike delay (+{extra_delay:.1f}h)"
                        )
                        strike_count += 1

            if strike_count > 0:
                print(
                    f"    [Risk] RSK-LOG-002: {strike_count} shipment legs delayed by port strike"
                )

        # Port congestion flicker quirk: AR(1) correlated delays
        if self.quirks_manager.is_enabled("port_congestion_flicker"):
            self.data["shipment_legs"] = self.quirks_manager.apply_port_congestion(
            ...
            congestion_count = sum(
                1
                for leg in self.data["shipment_legs"]
                if leg.get("congestion_affected")
            )
            if congestion_count > 0:
                print(
                    f"    [Quirk] Port congestion flicker: {congestion_count} legs with correlated delays"
                )
    ...
    def _generate_level_11(self) -> None:
        ...
        for sku in self.data["skus"]:
            ...
            try:
                size_val = float(
                    size_str.replace("oz", "").replace("ml", "").replace("g", "")
                )
                if "oz" in size_str:
                    sku_weights_kg[sku["id"]] = round(
                        size_val * 0.028 * 24, 2
                    )  # oz to kg, 24/case
                elif "ml" in size_str:
                    sku_weights_kg[sku["id"]] = round(
        ...
        for shipment in self.data["shipments"]:
            ...
            for line_num in range(1, num_lines + 1):
                ...
                sku_idx = random.choices(range(len(sku_ids)), weights=sku_weights_dist)[
                    0
                ]
                ...
                if line_num == num_lines:
                    ...
                    qty_cases = random.randint(
                        1, max(1, remaining_cases - (num_lines - line_num))
                    )
    ...
    def _generate_level_12(self) -> None:
        ...
        delivered_orders = [
            o for o in self.data["orders"] if o["status"] == "delivered"
        ]
        ...
        # Get approved RMAs
        approved_rmas = [
            r for r in self.data["rma_authorizations"] if r["status"] == "approved"
        ]
    ...
    def _generate_level_14(self) -> None:
        ...
        kpi_id_counter = 1
        for week_num in range(1, 53):
            measurement_date = date(self.base_year, 1, 1) + timedelta(
                weeks=week_num - 1
            )
        ...
        for table_name, records in audit_tables:
            ...
            for _ in range(sample_size):
                ...
                changed_at = self.fake.date_time_between(
                    start_date=datetime(self.base_year, 1, 1), end_date=now
                )
    ...
    def generate_all(self) -> None:
        ...
        # Phase 6: Chaos injection status
        print("Chaos Injection Active:")
        print(f"  Risk events: {self.triggered_events}")
        print(f"  Quirks enabled: {self.quirks_manager.get_enabled_quirks()}")
        print(f"  Stochastic mode: {self.stochastic_mode.value}")
        print()
    ...
    def validate_realism(self) -> dict[str, tuple[bool, str]]:
        ...
        # Chaos injection (risk events + quirks)
        results["chaos_injection"] = self._validate_chaos_injection()
    ...
    def _validate_chaos_injection(self) -> tuple[bool, str]:
        ...
        if self.risk_manager.is_triggered("RSK-BIO-001"):
            recall_batch = next(
                (
                    b
        ...
        if self.risk_manager.is_triggered("RSK-CYB-004"):
            chicago_dc_id = self.dc_ids.get("DC-NAM-CHI-001")
            if chicago_dc_id:
                hold_waves = [
                    w
    ...
    def write_sql(self, output_path: Path = OUTPUT_PATH) -> None:
        ...
        with open(output_path, "w") as f:
            ...
            for table_name, rows in self.data.items():
                ...
                if tables_written % 10 == 0:
                    ...
                    print(
                        f"    Writing... {pct:.0f}% ({tables_written}/{table_count} tables)"
                    )
        ...
        print(
            f"Done. {total_rows:,} rows written in {write_elapsed:.2f}s ({rows_per_sec:,.0f} rows/sec)"
        )
